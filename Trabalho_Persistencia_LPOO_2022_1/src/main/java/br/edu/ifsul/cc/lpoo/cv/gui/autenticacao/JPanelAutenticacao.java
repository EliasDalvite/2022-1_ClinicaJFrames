...
public class JPanelAutenticacao extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getActionCommand().equals(btnLogar.getActionCommand())){
            ...
            else if(txfNickname.getText().trim().length() <= 4) {
                JOptionPane.showMessageDialog(this, "Erro, informe nome com ao menos 4 caracteres");
            }
            else if(new String(psfSenha.getPassword()).trim().length() <= 3){
                JOptionPane.showMessageDialog(this, "Erro, informe senha com ao menos 3 digitos");
            }
            else {
                JOptionPane.showMessageDialog(this, "Informe os dados para Nome e Senha!", "Autenticação", JOptionPane.ERROR_MESSAGE);
